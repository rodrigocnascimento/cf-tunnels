...
| `run.sh` | Route persistent-zone writes through the shared registration path if approved by DG-03 |
...
If DG-03 is approved, these forms use the same registration operation:
...
After `cloudflared tunnel login` succeeds, `cftunnel` validates the root
`cert.pem`. Only a valid matching certificate is installed as:
...
must not replace the destination certificate, and must not silently delete the
rejected root credential.
...
Legacy uppercase or trailing-dot defaults are addressed by DG-02.
...
After `cloudflared tunnel login` exits successfully:

1. Require a regular, readable root `cert.pem`.
2. Parse an X.509 certificate with `openssl x509 -noout`.
3. Reject a certificate that fails `openssl x509 -checkend 0 -noout`.
4. Confirm hostname coverage with `openssl x509 -checkhost <zone> -noout`.
5. Extract Subject Alternative Name DNS entries without printing other
   certificate material.
6. Require a non-wildcard DNS SAN exactly equal to the canonical zone.
7. Atomically replace the destination certificate only after all checks pass.
8. Enforce destination mode `600`.
...
### Exact-match decision gate

Before implementation, DG-01 must confirm that currently supported Cloudflare
login certificates reliably include a literal DNS SAN for the selected zone.
If they do not, implementation must stop for design review rather than silently
downgrading to wildcard coverage.
...
- delete a rejected root certificate automatically;
- overwrite a known-good destination before the replacement is fully
  validated and secured;
- relax permissions beyond `600`.

If a rejected credential remains at `~/.cloudflared/cert.pem`, the error must
identify that path and explain that the user should inspect the selected zone
and retry. The CLI must not guess whether a credential is safe to delete.

### Existing root credential

Because `cloudflared tunnel login` writes to the fixed root location and does
not expose an output-path flag, a pre-existing root `cert.pem` creates ambiguity
between a stale file and newly generated output.

The proposed safe default is to refuse `zone login` before opening the browser
when a root certificate already exists. The error should direct the user to
place that credential in its correct zone or explicitly back it up/remove it.
No automatic backup or deletion is included in this feature.
...
- Subject to DG-03, route `--persist` and the interactive “make default” path
  through `register_zone()`.
...
- Remove the root source only after installation succeeds.
...
| `test_persist_uses_registration_contract` | If DG-03 is approved, `--persist` also creates the directory |
...
| `test_zone_login_refuses_preexisting_root_cert` | Ambiguous root state is preserved and login does not start |
...
- Add troubleshooting for mismatched, expired, malformed, and pre-existing root
  certificates.
...
| Normalization orphans an existing mixed-case directory | Low | Medium | Resolve DG-02 and test legacy behavior before implementation |
...
| A stale root certificate is mistaken for fresh login output | Medium | High | Refuse ambiguous pre-existing root state |
...
| Migrating or renaming existing noncanonical directories automatically | Potentially destructive and must be explicitly designed if DG-02 finds real usage |
...
was active. If canonicalization changes persisted values, the release notes must
record the manual restoration procedure established by DG-02.
...
**Question:** Do all supported `cloudflared tunnel login` certificates contain
a literal non-wildcard DNS SAN equal to the zone selected in the browser?

**Recommended decision:** Verify with supported current certificates without
recording secrets. If not guaranteed, pause and revise the certificate-binding
contract; do not silently accept wildcard-only identity.

### DG-02: Legacy normalization

**Question:** How should existing uppercase or terminal-dot `.default_zone`
values and directories behave?

**Recommended decision:** Normalize newly supplied values. On load, accept a
legacy value only when its canonical directory is unambiguous; otherwise fail
with a manual migration message. Never automatically rename a populated
directory.

### DG-03: `--persist` registration

**Question:** Should every path that persists a default also create the zone
directory?

**Recommended decision:** Yes. Route `zone use`, `--persist`, and interactive
default changes through one registration helper so the invariant remains true:

```text
default zone exists ⇒ matching zone directory exists
```

### DG-04: Root certificate preflight

**Question:** Should `zone login` refuse to run when root `cert.pem` already
exists?

**Recommended decision:** Yes for this feature. Refusing is safer than moving,
overwriting, or deleting a credential whose origin is ambiguous. A future
explicit import/migration command can handle existing root certificates.
